...
public class OneM2MWorker {
    ...
    public interface StateListener {
        ...
        void onSensorInfoReceived(contentInstanceResponse sensorInfo);
    }
    ...
    public void connect(Context context, final boolean registerDevice) {
        ...
        mqttService = MQTTClient.connect(IMQTT.class, config, mBinder, new MQTTProcessor.MQTTListener() {
                    ...
                    @Override
                    public void onPush(contentInstanceResponse sensorInfo) {
...
//        String mgmtCmdName = deviceID + "_" + Const.MGMTCMD_NAME;
    ...
    public void setSubscription() {
        // create subscription
        UserInfo userInfo = UserInfo.getInstance(OneM2MWorker.this.context);
        String appEUI = userInfo.loadAppEUI();
        String deviceID = userInfo.loadDeviceID();
        String to = "/" + appEUI + Const.ONEM2M_TO + "/remoteCSE-" + deviceID + "/container-" + Const.CONTAINER_LORA_NAME;

        oneM2MAPI.getInstance().tpRegisterSubscription(mqttService, Const.CONTAINER_LORA_NAME, to, uKey, "1", "2", new MQTTCallback<subscriptionResponse>() {
            @Override
    ...
    public void removeSubscription(MQTTCallback callback) {
        ...
        String deviceID = userInfo.loadDeviceID();
        String to = "/" + appEUI + Const.ONEM2M_TO + "/remoteCSE-" + deviceID + "/container-" + Const.CONTAINER_LORA_NAME
                + "/subscription-" + Const.CONTAINER_LORA_NAME;

        oneM2MAPI.getInstance().tpRegisterSubscription(mqttService, Const.CONTAINER_LORA_NAME, to, uKey, "1", "2", callback);
        try {
