...
public class oneM2MAPI {
     ...
     * @param to                to
     * @param uKey              userKey
     * @param rss               resourceStatus(notification condition - 1 : childCreated, 2 : childDeleted, 3 : Updated, 4 : Deleted, 5: childUpdated)
     * @param nct               notificationContentType(1 : Modified Attreibute only, 2 : Whole Resource)
     * @param callback          response callback
     */
    public void tpRegisterSubscription(final IMQTT mqttService, final String subscriptionName, final String to, final String uKey,
                                    final String rss, final String nct, final MQTTCallback callback) {
        try {
            MQTTUtils.checkNull(mqttService, "IMQTT is null!");
            MQTTUtils.checkNull(callback, "MQTTCallback is null!");

            subscription subscriptionCreate = new subscription.Builder(Definitions.Operation.Create).
                    nm(subscriptionName).
                    to(to).
                    uKey(uKey).
                    rss(rss).
                    nu("MQTT|{" + MQTTConst.CLIENT_ID + "}").
                    nct(nct).build();

            mqttService.publish(subscriptionCreate, new MQTTCallback<subscriptionResponse>() {
                @Override
