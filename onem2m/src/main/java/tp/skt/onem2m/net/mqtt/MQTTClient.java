                    ...
                    if (topic.startsWith("/oneM2M/req")) {
                        try {
                            ...
                            if(result.contains("<sr>")) {
                                contentInstanceResponse notification = mqttProcessor.parsing(contentInstanceResponse.class, result);
                                notification.print();
                                mqttListener.onPush(notification);
                            } else {
                                execInstanceControl control = mqttProcessor.parsing(execInstanceControl.class, result);
                                control.print();
